...
Map<Function, double> getPatternGenerators() {
  ...
  if (estats.isPlainChosen) {
    map.addAll({plain: 1});
  }
  if (estats.isYWallChosen) {
    map.addAll({yWall: 1});
  }
  if (estats.isXWallChosen) {
    map.addAll({xWall: 1});
  }
  if (estats.isPlainHardChosen) {
    map.addAll({plainHard: 1});
  }
  if (estats.isYCrossChosen) {
    map.addAll({yCross: 1});
  }
  if (estats.isLaserRoomChosen) {
    map.addAll({laserRoom: 1});
  }
  if (estats.isPortalsChosen) {
...
bool laserRoom(int width, int height, List<Pattern> patterns) {
  if (height >= 6 && width >= 5) {
    ...
    for (var i = 0; i < width; i++) {
      ...
      if (i != upperLaserX) {
        _mobs.add(Border(_mobs.length, Point(i, 0)));
      }

      if (i != lowerLaserX) {
        _mobs.add(Border(_mobs.length, Point(i, height - 1)));
      }
    }
    ...
    _mobs.add(Annihilator(_mobs.length, Point(upperLaserX, 0), Directions.down,
        1, upperReload, []));

    _mobs.add(Annihilator(_mobs.length, Point(lowerLaserX, height - 1),
        Directions.up, 1, lowerReload, []));

    patterns.add(Pattern(_mobs, path, idealPath));
...
bool yWall(int width, int height, List<Pattern> patterns) {
  ...
  if (width % 2 == 1 && width > 4) {
    ...
    for (var i = 0; i < height; i++) {
      _mobs.add(Border(0, Point<int>(middle, i)));
      forbidden.add(Point<int>(middle, i));
    }
    var remove = Random().nextInt(_mobs.length);
    _mobs.removeAt(remove);
...
bool xWall(int width, int height, List<Pattern> patterns) {
  ...
  if (height % 2 == 1 && height > 4) {
    ...
    for (var i = 0; i < width; i++) {
      _mobs.add(Border(0, Point<int>(i, middle)));
...
bool yCross(int width, int height, List<Pattern> patterns) {
  ...
  if (height % 2 == 1 && width % 2 == 1 && height > 4 && width > 4) {
    ...
    for (var i = 0; i < width; i++) {
      _mobs.add(Border(0, Point<int>(i, ymiddle)));
    ...
    for (var i = 0; i < height; i++) {
      if (i != ygap1.y && i != ygap2.y) {
        _mobs.add(Border(0, Point<int>(xmiddle, i)));
...
bool portals(int width, int height, List<Pattern> patterns) {
  if (height % 2 == 1 && height > 4 && width > 4) {
    ...
    _mobs.add(Portal(_mobs.length, portalEntrance, [], true,
