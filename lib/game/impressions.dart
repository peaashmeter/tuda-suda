...
newRngSkinId() {
  List<int> own = [];
  for (var i = 0; i < stats.charSkinsOwn.length; i++) {
    if (i != 16) {
      own.add(stats.charSkinsOwn.indexWhere((e) => e == 'true', i));
    }
  }
  own = own.toSet().toList()
    ..removeWhere((element) => element == -1)
    ..removeWhere((element) => element == 16);
  var rng = own[Random().nextInt(own.length)];
  rngSkinId = rng;
}
  ...
  _getImpression([int parameter = 0]) {
    switch (impression) {
      case Impressions.arrowMob:
        return (getArrowMobImpression(
            stats.arrowMobSkinId, direction, widget.cellSize));
      case Impressions.player:
        return getPlayerImpression(stats.charSkinId);
      ...
      case Impressions.border:
        return getBorderImpression(
            stats.borderSkinId, parameter, widget.cellSize);
