...
import 'package:logicgame/sound_handler.dart' as audio;
...
class Player extends Mob {
  ...
  Widget getImpression(double size) {
    return Impression(
      isAnimated: settings.isMoreAnimations,
      cellSize: size,
      direction: direction,
      impression: impression,
      key: UniqueKey(),
  ...
  @override
  void move(int width, int height, List<Mob> bounds) {}
...
class Info extends Mob with Electric {
  ...
  showInfo(BuildContext context) {
    showDialog(
      context: context,
      builder: (context) => SkinDialog(id, 'note'.tr(), dialog.tr()),
    );
  }
